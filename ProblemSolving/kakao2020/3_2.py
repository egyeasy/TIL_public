...
def judge(lock_i_diff, lock_j_diff, len_lock, len_key, key, lock):
    ...
    for i in range(len_lock):
        for j in range(len_lock):
            if lock[i][j] == 2:
                lock[i][j] = 0
    print()
    print(lock_i_diff, lock_j_diff)
    for i in range(len_key):
        lock_i = lock_i_diff - (len_key - i) + 1
        if 0 <= lock_i < len_lock:
            for j in range(len_key):
                lock_j = lock_j_diff - (len_key - j) + 1
                if 0 <= lock_j < len_lock:
                    if key[i][j] == lock[lock_i][lock_j]:
                        return False
                    elif lock[lock_i][lock_j] == 1 and key[i][j] == 0:
                        pass
                    elif lock[lock_i][lock_j] == 0 and key[i][j] == 1:
                        print("lock:", lock_i, lock_j)
                        print("key:", i, j)
                        lock[lock_i][lock_j] = 2
                    else:
                        print(i, j, lock[lock_i][lock_j])
                        print("뭔가 잘못되었습니다")
