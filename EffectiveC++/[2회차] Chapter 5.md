...
## Item 31: 파일들 간의 컴파일 의존성을 최소화하라.

당신이 C++ 프로그램에서 클래스의 구현 부분에 약간의 변화를 주려고 한다고 가정하자.

구현에서의 private 관련해서 조금만 수정을 하고, 리빌드를 하면 시간이 얼마 안 걸릴 것이라고 가정한다.

하지만 빌드를 하거나 make를 입력하면 전체가 리컴파일 및 리링크 되는 것에 놀라게될 것이다.



### C++은 인터페이스를 구현과 분리하는 걸 잘하지 못한다

C++은 구현으로부터 인터페이스를 분리하는 데 능숙하지가 않다.

클래스 정의는 클래스 정의는 클래스 인터페이스 뿐만 아니라 상당 수의 구현 디테일까지를 지칭한다.
...
Person 클래스는 Person 구현이 사용하는 클래스들(string, Date, Address)의 정의에 접근하지 않고서는 컴파일될 수 없다.

그러한 정의는 일반적으로 #include 지시문을 이용해서 제공된다.
...
하지만 이로 인해 Person을 정의하는 파일과 이 헤더 파일들 간에 컴파일 의존성이 만들어지게 된다.

위 헤더 파일 중 하나라도 변경되면, 혹은 헤더 파일이 의존하는 헤더 파일 중 하나라도 변경되면, Person을 가지고 있는 파일 또한 리컴파일되어야 한다.

이러한 cascading 컴파일 의존성이 프로젝트에 큰 슬픔을 야기한다.



### 왜 C++은 클래스 정의에 구현 detail을 넣게 하는 것일까

왜 다음과 같이 클래스의 구현 디테일을 분리하는 방식으로 Person을 정의할 수 없는 것일까?:
...
이것이 가능하다면, Person의 클라이언트는 클래스에 대한 인터페이스가 변화했을 경우에만 리컴파일 하면 된다.

위 방식에는 두 가지 문제가 있다.

1. string은 클래스가 아니라 typedef이다(`basic_string<char>`에 대한).

   따라서 string에 대한 전방 선언은 틀렸다.

   적절한 전방 선언은 더 복잡한 방식인데, standard library의 부분을 수동으로 선언하는 것은 하면 안되므로 신경쓰지 않아도 된다.

   대신에 적절한 #include를 쓰고 만족해라.

   너의 빌드 환경이 precompiled 헤더를 지원한다면 standard 헤더는 컴파일 병목이 되기 어렵다.

   standard header parsing이 정말 문제라면 원치 않는 #include를 쓰는 부분을 피해서 인터페이스 디자인을 바꿀 필요가 있을 수도 있다.

2. 컴파일러는 컴파일 중에 객체의 사이즈를 알 필요가 있다.
   ...
   컴파일러가 x에 대한 정의를 마주쳤을 때는, int의 사이즈를 알고 있기 때문에 적당한 공간을 할당하고 int를 들고 있게 된다.

   컴파일러가 p에 대한 정의를 마주쳤을 때는, Person에 대한 충분한 공간을 할당해야 하는데 Person 객체가 얼마나 큰지 어떻게 알 수 있겠는가?

   유일한 방법은 class 정의를 보는 것이고,

   클래스 정의에서 구현 디테일을 생략해도 되는 게 합법이라면

   어떻게 컴파일러가 할당을 위해 충분한 공간의 크기가 얼마인지를 알 수 있겠는가?

   #### 포인터를 위한 공간만 할당하는 방식

   Smalltalk나 Java의 경우에는 객체가 정의될 때 컴파일러가 객체에 대한 **pointer**를 위한 충분한 공간만을 할당한다.

   C++에서도 "객체 구현을 포인터 뒤에 숨기는" 것을 너가 직접 할 수 있다.

   한 가지 방법은 Person을 두 개의 클래스로 나눠서, 한 클래스는 인터페이스만 제공하고 다른 클래스는 인터페이스에게 구현만 제공하도록 하는 것이다.
   ...
   여기서 메인 클래스(Person)은 포인터(클래스 구현을 가리키는)만 데이터 멤버로 가지고 있다.

   이러한 디자인을 **pimpl idiom(pointer to implementation)**이라고 부른다.



### pimpl idiom의 이점

이러한 디자인을 통해 Person의 클라이언트는 date, address, person과 같은 디테일로부터 분리될 수 있다.

그 디테일 구현들은 자유로 변경될 수 있지만 Person은 리컴파일 될 필요가 없어진다.

게다가 클라이언트는 Person의 구현 디테일을 볼 수 없기 때문에 그러한 detail에 의존하는 코드를 쓸 가능성도 적어지게 된다.

이것이 인터페이스와 구현의 진정한 분리이다.



분리의 핵심은 **'정의의 의존성'을 '선언의 의존성'으로 대체**하는 것이고, 이것이 컴파일 의존성을 줄이는 본질이다.

헤더 파일이 자급자족할 수 있도록 만드는 게 실용적이라면 언제든 그렇게 하고, 그렇지 않을 경우 다른 파일들의 '선언'(정의가 아니라)에 의존해라.

따라서,

- **객체 레퍼런스와 포인터로 충분할 때는 객체를 쓰는 것을 피해라.**

  타입에 대한 선언만을 가지고 타입에 대한 레퍼런스와 포인터를 정의할 수 있다.

  타입의 '객체'를 정의하는 것은 타입의 정의를 필요로 한다.

- **가능한 한 클래스 정의보다는 클래스 선언에 의존해라.**

  클래스를 쓰는 함수를 선언하기 위해 클래스 정의가 필요하지 않다는 것을 주지해라.

  그 함수가 클래스 타입을 by value로 pass하거나 return하더라도 말이다.

  물론 pass-by-value는 보통 나쁜 아이디어이기도 하다(Item 20).

  하지만 어떤 이유로 pass-by-value를 써야 한다고 하더라도, 불필요한 컴파일 의존성을 도입하는 것을 정당화할 수는 없다.
  

  객체의 정의 없이 함수를 선언하는 것이 놀라울 수도 있다.

  하지만 잘 생각해보면, 아무도 호출하지 않는 함수의 선언에 신경쓸 필요는 없다.

  라이브러리에 수십 개의 함수 선언이 있다고 해도, 모든 클라이언트가 모든 함수를 호출할 가능성은 낮다.

  **클래스 정의를 제공해야하는 책임**을 '함수 선언을 가지고 있는 헤더 파일'에서 '함수 호출을 가지고 있는 클라이언트 파일'로 옮김으로써,
...
순수 가상 함수를 가지고 있는 클래스를 instantiate하는 것은 불가능하기 때문에, 이 클래스의 클라이언트는 Person의 포인터와 레퍼런스를 가지고 프로그램해야 한다.(Person으로부터 파생된 클래스를 instantiate하는 것은 가능하다)

Handle 클래스의 클라이언트와 같이, 인터페이스 클래스의 클라이언트는 인터페이스가 변경되지 않는다면 리컴파일할 필요가 없다.



### 팩토리 메소드

인터페이스 클래스의 클라이언트에게는 새로운 객체를 만들 방법이 있어야 한다.

이것은 일반적으로, 실제로 인스턴스화된 파생 클래스의 생성자와 같은 역할을 하는 '함수'를 호출함으로써 이루어진다.

이 함수를 **factory 함수** 또는 virtual constructor라고 부른다.

팩토리 함수는 동적으로 할당된 객체에 대한 포인터를 반환하고, 보통 인터페이스 클래스 내부에서 static으로 선언된다.

```c++
class Person {
  public:
    static std::shared_ptr<Person> create(const ...);
};
```
...
물론 어느 한 시점에서 클래스의 인터페이스를 지지하는 구체적인 클래스가 정의되고, 실제 생성자가 호출되어야 한다.

이것은 가상생성자의 구현을 가지고 있는 파일 내에서 보이지 않게 일어난다.

예를 들어 Person이라는 인터페이스 클래스는 RealPerson이라는 구체적인 파생 클래스를 가지고 있고, RealPerson은 자신이 상속하는 가상 함수에 대한 구현을 가지고 있다.

```c++
std::shared_ptr<Person> Person::create(const ...) {
    return std::shared_ptr<Person>(new RealPerson(name, birthday, addr));
}
```

Person::create의 현실적인 구현은, 추가 함수 파라미터의 값이나 file/database로부터 읽어온 데이터, 환경 변수에 따라 다른 여러 타입의 파생 클래스 객체를 생성할 수도 있다.



RealPerson은 인터페이스 클래스를 구현하는 두 가지 일반적인 메커니즘 중 하나를 보여준다: 인터페이스 클래스(Person)로부터 인터페이스 스펙을 상속받고, 인터페이스의 함수를 구현한다.

두 번째 방법은 다중 상속을 포함하는 인터페이스 클래스를 구현하는 것이다.(Item 40)



### Handle 클래스, Interface 클래스의 단점

핸들 클래스와 인터페이스 클래스는 구현으로부터 인터페이스를 분리한다 -> 파일 간의 컴파일 의존성을 줄인다.

이렇게 됐을 때의 비용은 런타임의 속도 저하, 객체 당 메모리 사용량 증가가 있다.



#### 핸들 클래스

1. 핸들 클래스의 경우 멤버 함수들은 객체의 데이터에 접근하기 위해 구현 pointer를 거쳐야 한다.

   이 때문에 엑세스를 할 때마다 하나의 간접적인 레벨을 거치도록 된다.

2. 또한 각 객체를 저장하는 데 필요한 메모리의 양에 구현 포인터의 사이즈를 더해야 한다.

3. 마지막으로, 구현 포인터가 동적으로 할당된 구현 객체를 가리키도록 초기화되어야 하므로, 동적인 메모리 할당에 내재된 오버헤드, 그리고 bad-alloc(메모리 부족) exception을 만날 가능성이 발생한다.



#### 인터페이스 클래스

1. 인터페이스 클래스의 경우 모든 함수가 virtual이므로, 함수를 호출할 때마다 indirect jump가 발생한다.

2. 또한 인터페이스 클래스의 파생 클래스는 virtual table pointer를 가지고 있어야 한다 -> 객체를 저장하는 데 더 많은 메모리 공간이 필요하게 된다.



마지막으로, 핸들 클래스나 인터페이스 클래스는 inline 함수에서 사용될 수 없다. 인라인 함수는 일반적으로 헤더 파일에 function body가 오도록 하지만, 핸들 클래스와 인터페이스 클래스는 function body와 같은 구현 디테일을 숨기도록 디자인 된다.
